...
python3 ./script/dns-script/singbox.py
python3 ./script/dns-script/surge.py

curl --fail --location --retry 3 --output ssc \
  https://github.com/PuerNya/sing-srs-converter/releases/download/v2.0.1/sing-srs-converter-v2.0.1-linux-x86_64_v3
chmod +x ssc
./ssc adrules.list -m
mv adrules.list.srs adrules-singbox.srs

# 转换adrules_domainset.txt为clash.mrs(mihomo)
mihomo_version=$(curl --fail --location --retry 3 \
  https://github.com/MetaCubeX/mihomo/releases/latest/download/version.txt)
curl --fail --location --retry 3 --output "mihomo-linux-amd64-$mihomo_version.gz" \
  "https://github.com/MetaCubeX/mihomo/releases/latest/download/mihomo-linux-amd64-$mihomo_version.gz"
